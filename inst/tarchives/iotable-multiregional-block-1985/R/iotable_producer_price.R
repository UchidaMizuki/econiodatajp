...
target_iotable_producer_price <- tar_plan(
  tar_change(
    file_iotable_nation_45_producer_price_competitive_import_ja,
    download_file_meti(
      url = "https://www.meti.go.jp/statistics/tyo/tiikiio/result/result_3/xlsx/h2rio85a.xlsx",
      destfile = "_targets/user/iotable_nation_45_producer_price_competitive_import_ja.xlsx"
    ),
    change = "0.1.0",
    format = "file"
  ),
  iotable_nation_45_producer_price_competitive_import_ja = read_file_iotable_producer_price(
    file = file_iotable_nation_45_producer_price_competitive_import_ja,
    sheet = "取引額（45部門MTX）"
  )
)
