target_sector <- tar_plan(
  tar_change(
    # https://www.e-stat.go.jp/stat-search/files?page=1&layout=datalist&toukei=00200603&tstat=000001073129&cycle=0&tclass1val=0&metadata=1&data=1
    file_sector_ja,
    download_file(
      url = "https://www.e-stat.go.jp/stat-search/file-download?statInfId=000030137957&fileKind=0",
  ...
  tar_change(
    ...
    file_sector_en,
    download_file(
      url = "https://www.e-stat.go.jp/en/stat-search/file-download?statInfId=000030137957&fileKind=0",
...
read_file_sector <- function(file_ja, file_en) {
  ...
  sector_en <- bind_rows(
    ...
    readxl::read_excel(
      file_en,
      sheet = "Final Demand・Gross Valued Added",
      col_names = col_names,
      col_types = "text"
    ) |>
      filter(
...
get_sector <- function(sector_raw, axis) {
  ...
  if (axis == "output") {
    ...
    # lists a handful of industry codes (e.g. scrap-material memo items)
    # only on the input side, leaving no output-side small/medium/large/
    # template grouping for them. Backfill those from the input axis so
    # `sector_output` doesn't miss sectors that the real table's output
    # dimension does have. Verified (2020 data) that every other industry
    # code already classifies identically on both axes at every
    # sector_class, so this only adds rows, never replaces one.
