target_sector <- tar_plan(
  ...
  tar_change(
    ...
    # ("令和2年（2020年）産業連関表基本分類－日本標準産業分類（平成25年
    # （2013年）改定）細分類対応表").
    file_sector_jsic_xlsx,
    download_file(
  ...
  sector_jsic_raw = read_file_sector_jsic_xlsx(file_sector_jsic_xlsx),
  ...
  tar_change(
    ...
    # file_sector_jsic_xlsx above, this doesn't reference IO sectors at
    # all). Both this and the 2015 pipeline reuse the same file since both
